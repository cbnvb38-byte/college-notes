-- Migration: 012_phase7_repair.sql
-- Description: Safe repair script that creates/updates Phase 7 discovery objects without data loss.
...
-- ==========================================
-- FUNCTION: get_trending_notes (Safe Fallback)
-- ==========================================
...
BEGIN
  ...
  SELECT 
    ...
    b.code AS branch_code,
    (
      COALESCE(n.downloads_count, 0) * 3 +
      COALESCE(n.view_count, 0) +
      COALESCE(n.bookmarks_count, 0) * 2 +
      COALESCE(n.total_ratings, 0) * 2 +
      COALESCE(n.average_rating, 0) * 5
